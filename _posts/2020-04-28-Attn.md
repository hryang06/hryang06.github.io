...
$$c_t =  \sum_{j=1}^T \alpha_{t,j} c_{t,j}$$
...
$$e_t^{(i,j)} = tanh(R_{g_t} + S F_t^{(i,j)} + b)$$ (11)
...
$$L = (1 - \lamda)L_{Att} + \lamda L_{focus}$$
- $$\lamda (0 <= \lamda < 1)$$ : tunable parameters, AN과 FN loss의 weight를 결정한다.
